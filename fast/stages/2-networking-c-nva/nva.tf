...
data "template_file" "bgp_configs" {
  for_each = local.nva_configs
  template = file("data/bgp-config.tpl")
  vars = {
    # Each NVA announces its trusted regional subnets
    announce-to-nva = (
      each.value.name == "primary"
      ? "PRIMARY"
      : "SECONDARY"
    )
    ...
    cost_primary                    = each.value.name == "primary" ? "100" : "10100"
    cost_secondary                  = each.value.name == "primary" ? "10100" : "100"
    ...
    ip_neighbor_cross_region_nva_0  = local.nva_configs[each.key].ip_neighbor_cross_region_nva_0
    ip_neighbor_cross_region_nva_1  = local.nva_configs[each.key].ip_neighbor_cross_region_nva_1
    ip_neighbor_trusted_0           = local.nva_configs[each.key].ip_neighbor_trusted_0
    ip_neighbor_trusted_1           = local.nva_configs[each.key].ip_neighbor_trusted_1
    ip_neighbor_untrusted_0         = local.nva_configs[each.key].ip_neighbor_untrusted_0
    ip_neighbor_untrusted_1         = local.nva_configs[each.key].ip_neighbor_untrusted_1
    ip_trusted                      = local.nva_configs[each.key].ip_trusted
    ip_untrusted                    = local.nva_configs[each.key].ip_untrusted
  }
}
...
module "nva-bgp-cloud-config" {
  ...
  frr_config = {
    config_file     = data.template_file.bgp_configs[each.key].rendered
